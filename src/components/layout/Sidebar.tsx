...
export function Sidebar() {
    ...
    if (isManager) {
        menuItems.push({ icon: UserPlus, label: 'Kullanıcı Davet', href: '/admin/invite', auth: true })
    }
    ...
    return (
        <aside className="fixed left-0 top-0 h-screen w-20 md:w-64 border-r border-gray-200 dark:border-gray-800 hidden md:flex flex-col items-center md:items-start p-4 bg-white dark:bg-black z-50">
            <div className="mb-8 p-2">
                <h1 className="text-2xl font-bold hidden md:block italic">GTATweet</h1>
            ...
            {user ? (
                <>
                    ...
                    {isManager ? (
                        <div className="mt-4 w-full space-y-2">
                            <button onClick={toggleAiMode} className={`w-full flex items-center justify-center gap-2 ${isAiMode ? 'bg-gradient-to-r from-purple-600 to-blue-600 text-white' : 'bg-gray-800 text-white'} font-bold py-3 rounded-full shadow-lg`}>
                    ...
                    <div className="mt-4 w-full">
                        <button onClick={() => setBugOpen(true)} className="w-full flex items-center justify-center gap-2 bg-white/0 border border-gray-300 dark:border-gray-800 text-gray-700 dark:text-gray-300 py-2 rounded-full hover:bg-gray-50 dark:hover:bg-gray-900 transition-all">
                ...
                <div className="space-y-2 w-full">
                    <Link href="/login" className="flex items-center justify-center p-3 rounded-full bg-blue-600 text-white font-bold w-full hover:opacity-90 transition-colors shadow-lg shadow-blue-500/20">
                        Giriş Yap
                    </Link>
